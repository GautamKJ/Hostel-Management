...
function Admin_addStudent() {
  ...
  const onsubmit=async(e)=>{
    ...
    try {
      ...
      const response=await fetch("https://hostels-management.onrender.com/api/addstudent",{
        method:"POST",
        headers:{
      ...
      if(response.status==200){
        setLoading(true);
        try{
          const response=await fetch("https://hostels-management.onrender.com/api/sendmail",{
            method:"POST",
            headers:{
              'content-Type':'application/json',
              'auth-token':localStorage.getItem('token')
